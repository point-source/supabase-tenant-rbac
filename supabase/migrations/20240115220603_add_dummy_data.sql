...
create policy "Allow group admins to modify"
on "public"."sensitive_data"
as permissive
for all
to authenticated
using (has_group_role(owned_by_group, 'admin'::text))
with check (has_group_role(owned_by_group, 'admin'::text));


create policy "Allow group member to read"
on "public"."sensitive_data"
as permissive
for select
to authenticated
using (is_group_member(owned_by_group));
