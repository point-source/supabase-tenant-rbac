...
-- Test 2: get_user_claims() returns NULL when request.groups is empty string
-- (falls back to JWT which also has no groups)
SELECT set_config('request.groups', '', true);
SELECT is(
    get_user_claims(),
    NULL::jsonb,
    'get_user_claims() returns NULL when request.groups is empty string and JWT has no groups'
);
